#!/usr/bin/env python3
"""
make_waveform.py - render docs/axil_regfile_waveform.png from a REAL simulator run.

This parses the VCD produced by Icarus Verilog (`tb_axil_dump.vcd`, from
`make icarus_dump`) and draws a cycle-accurate digital timing diagram of the
AXI4-Lite "showcase" window: reset release, a write (AW/W/B) to reg1, a read
(AR/R) of reg1 returning the written data, and an out-of-range write/read that
returns SLVERR.

The waveform is therefore captured from a genuine simulation, not hand-modeled.

Usage:
    make icarus_dump          # produces tb_axil_dump.vcd
    python3 docs/make_waveform.py [path/to/tb_axil_dump.vcd]
"""
import sys, os
import matplotlib
matplotlib.use("Agg")
...
VCD  = sys.argv[1] if len(sys.argv) > 1 else os.path.join(HERE, "..", "tb_axil_dump.vcd")
OUT  = os.path.join(HERE, "axil_regfile_waveform.png")
...
WANT = ["ACLK", "ARESETn",
        ...
        "RDATA", "RRESP", "RVALID"]
BUS  = {"AWADDR", "WDATA", "WSTRB", "BRESP", "ARADDR", "RDATA", "RRESP"}
        ...
        for line in f:
            ...
            if defining:
                if line.startswith("$scope"):
                    depth += 1
                    in_top = (depth == 1)          # tb_axil_dump is scope depth 1
...
def main():
    ...
    # Time window: reset release + showcase transactions. Timescale is 1 ps.
    PS_PER_NS = 1000
    t0, t1 = 35 * PS_PER_NS, 215 * PS_PER_NS
    step = 500
    ts = list(range(t0, t1 + step, step))

    fig, axes = plt.subplots(len(WANT), 1, figsize=(13, 10.6), sharex=True)
    fig.suptitle("axil_regfile - AXI4-Lite write / read / out-of-range(SLVERR)\n"
                 "captured from Icarus Verilog (tb_axil_dump.vcd) - REAL simulation",
                 fontsize=12, fontweight="bold")

    # Clock-cycle gridlines (posedges at 5ns + k*10ns).
    posedges = [ (5 + 10*k) * PS_PER_NS for k in range(0, 24) ]
        ...
        if name in BUS:
            ...
            def draw_seg(a, b, valstr):
                ...
                if stable and iv != 0:
                    ...
                    label = "0x%0*X" % (max(1, (len(valstr)+3)//4), iv)
            ...
            for t in ts:
                ...
                ys.append(lvl)
            color = "#1f4e8c" if name == "ACLK" else "#0b6b3a"
