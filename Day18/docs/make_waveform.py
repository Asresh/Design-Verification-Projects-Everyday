#!/usr/bin/env python3
"""
make_waveform.py - render docs/warp_scan_waveform.png from a REAL sim run.

Parses the VCD produced by Icarus Verilog (`tb_warp_scan_dump.vcd`, from
`make icarus_dump`) and draws the DIRECTED SHOWCASE of the GPU warp-level
parallel prefix-sum (scan) engine: an INCLUSIVE scan of the ramp 1,2,...,8.

    input lane 0..7 (each 16-bit):
        in0=1 in1=2 in2=3 in3=4 in4=5 in5=6 in6=7 in7=8

LAT = 5 cycles later the eight output lanes come out as the running TRIANGULAR
NUMBERS (out[i] = sum of in[0..i]):

        out0=1  out1=3  out2=6  out3=10
        out4=15 out5=21 out6=28 out7=36

The story the window tells: an 8-lane vector is presented for one cycle
(in_valid pulse, in_excl=0), streams through the 3-layer Kogge-Stone pipeline,
and exactly LAT cycles later out_valid pulses with the fully scanned vector -
each lane holding the running cumulative sum up to and including that lane.

The trace is captured from a genuine Icarus simulation - it is NOT hand-modeled.

Usage:
    make icarus_dump          # produces tb_warp_scan_dump.vcd
    python3 docs/make_waveform.py [path/to/tb_warp_scan_dump.vcd]
"""
import sys, os
import matplotlib
matplotlib.use("Agg")
...
VCD  = sys.argv[1] if len(sys.argv) > 1 else os.path.join(HERE, "..", "tb_warp_scan_dump.vcd")
OUT  = os.path.join(HERE, "warp_scan_waveform.png")

# Display order (top -> bottom).
WANT = ["clk", "in_valid", "in_excl",
        "in0", "in1", "in2", "in3", "in4", "in5", "in6", "in7",
        "out_valid", "out_excl",
        "out0", "out1", "out2", "out3", "out4", "out5", "out6", "out7"]
LEVEL  = {"clk", "in_valid", "in_excl", "out_valid", "out_excl"}
# name -> nibble width for fixed-width hex buses (DW=16 -> 4 nibbles)
HEXBUS = {n: 4 for n in
          ["in0", "in1", "in2", "in3", "in4", "in5", "in6", "in7",
           "out0", "out1", "out2", "out3", "out4", "out5", "out6", "out7"]}
        ...
        for line in f:
            ...
            if defining:
                if line.startswith("$scope"):
                    depth += 1
                    in_top = (depth == 1)          # tb_warp_scan_dump = depth 1
...
def main():
    ...
    fig, axes = plt.subplots(len(WANT), 1, figsize=(13, 13.6), sharex=True)
    fig.suptitle(
        "warp_scan - GPU warp-level parallel prefix-sum (Kogge-Stone scan) showcase\n"
        "REAL Icarus Verilog capture (tb_warp_scan_dump.vcd), clk 10 ns - inclusive "
        "scan of ramp 1..8 -> out lanes 1,3,6,10,15,21,28,36 (triangular), LAT=5",
    ...
    for ax, name in zip(axes, WANT):
        ...
        ax.set_ylabel(name, rotation=0, ha="right", va="center",
                      fontsize=8.6, labelpad=26, fontfamily="monospace")
        ax.set_yticks([])
        ...
        if name in HEXBUS:
            # tint input lanes vs output lanes differently for readability
            ...
            if name == "clk":
                ...
                color = "#1f7a4d"      # green: scanned-vector strobe
            else:                      # in_excl / out_excl
                color = "#d98a00"      # amber: scan mode
