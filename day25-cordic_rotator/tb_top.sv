// -----------------------------------------------------------------------------
// tb_top.sv - UVM top for the rotation-mode CORDIC engine. Instantiates the
// clock/reset, the interface, and the DUT, wires them together, publishes the
// virtual interface to the config DB, and calls run_test(). Select the test
// with +UVM_TESTNAME=cordic_smoke_test | cordic_regress_test.
//
// Needs a UVM-capable simulator (VCS / Questa / Verilator >= 5 --uvm). For the
// open-source Icarus flow use tb_cordic_rotation_dump.sv (see the Makefile).
// -----------------------------------------------------------------------------
`timescale 1ns/1ps

module tb_top;
    import uvm_pkg::*;
    import cordic_rotation_pkg::*;

    localparam int DW    = 16;
    localparam int AW    = 16;
    localparam int FRAC  = 13;
    localparam int NITER = 16;

    logic clk;
    logic rst_n;

    initial clk = 1'b0;
    always #5 clk = ~clk;    // 100 MHz

    initial begin
        rst_n = 1'b0;
        repeat (4) @(posedge clk);
        rst_n = 1'b1;
    end

    cordic_rotation_if #(.DW(DW), .AW(AW)) vif (.clk(clk), .rst_n(rst_n));

    cordic_rotation #(.DW(DW), .AW(AW), .FRAC(FRAC), .NITER(NITER)) dut (
        .clk       (clk),
        .rst_n     (rst_n),
        .in_valid  (vif.in_valid),
        .in_x      (vif.in_x),
        .in_y      (vif.in_y),
        .in_angle  (vif.in_angle),
        .out_valid (vif.out_valid),
        .out_x     (vif.out_x),
        .out_y     (vif.out_y),
        .out_angle (vif.out_angle)
    );

    initial begin
        uvm_config_db#(virtual cordic_rotation_if)::set(null, "*", "vif", vif);
        $dumpfile("tb_top.vcd");
        $dumpvars(0, tb_top);
        run_test();
    end

    // Global watchdog independent of UVM objections.
    initial begin
        #1ms;
        `uvm_fatal("TIMEOUT", "global watchdog fired")
    end
endmodule
