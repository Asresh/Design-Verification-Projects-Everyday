#!/usr/bin/env python3
"""
make_waveform.py - render docs/ral_regblock_waveform.png from a REAL simulator run.

This parses the VCD produced by Icarus Verilog (`tb_ral_dump.vcd`, from
`make icarus_dump`) and draws the APB "showcase" window of the RAL demo: a write
to the RW CTRL register, a read-back of CTRL, and a read of the read-only STATUS
register returning its hardware reset value 0xDEADBEEF.

The waveform is therefore captured from a genuine simulation, not hand-modeled.

Usage:
    make icarus_dump          # produces tb_ral_dump.vcd
    python3 docs/make_waveform.py [path/to/tb_ral_dump.vcd]
"""
import sys, os
import matplotlib
matplotlib.use("Agg")
...
VCD  = sys.argv[1] if len(sys.argv) > 1 else os.path.join(HERE, "..", "tb_ral_dump.vcd")
OUT  = os.path.join(HERE, "ral_regblock_waveform.png")

WANT = ["PCLK", "PRESETn", "PSEL", "PENABLE", "PWRITE",
        "PADDR", "PWDATA", "PRDATA", "PSTRB", "PREADY", "PSLVERR"]
BUS  = {"PADDR", "PWDATA", "PRDATA", "PSTRB"}
        ...
        for line in f:
            ...
            if defining:
                if line.startswith("$scope"):
                    depth += 1
                    in_top = (depth == 1)          # tb_ral_dump is scope depth 1
...
def main():
    ...
    # Time window: showcase transfers (15 .. 140 ns). Timescale is 1 ps.
    PS_PER_NS = 1000
    t0, t1 = 15 * PS_PER_NS, 140 * PS_PER_NS
    step = 500
    ts = list(range(t0, t1 + step, step))

    fig, axes = plt.subplots(len(WANT), 1, figsize=(13, 8.6), sharex=True)
    fig.suptitle("ral_regblock - APB RAL demo: write CTRL / read CTRL / read RO STATUS\n"
                 "captured from Icarus Verilog (tb_ral_dump.vcd) - REAL simulation",
                 fontsize=12, fontweight="bold")

    posedges = [ (5 + 10*k) * PS_PER_NS for k in range(0, 15) ]
    ...
    # Annotate the three transfers.
    ann = axes[0]
    for x, txt in [(56, "WRITE CTRL"), (86, "READ CTRL"), (116, "READ STATUS")]:
