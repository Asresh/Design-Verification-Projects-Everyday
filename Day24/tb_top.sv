// -----------------------------------------------------------------------------
// tb_top.sv - UVM top for the SECDED (72,64) Hamming ECC encoder/decoder.
// Instantiates the clock/reset, the interface, and the DUT, wires them together,
// publishes the virtual interface to the config DB, and calls run_test(). Select
// the test with +UVM_TESTNAME=secded_smoke_test | secded_regress_test.
//
// Needs a UVM-capable simulator (VCS / Questa / Verilator >= 5 --uvm). For the
// open-source Icarus flow use tb_secded_ecc_dump.sv (see the Makefile).
// -----------------------------------------------------------------------------
`timescale 1ns/1ps

module tb_top;
    import uvm_pkg::*;
    import secded_ecc_pkg::*;

    localparam int DW  = 64;
    localparam int HAM = 7;
    localparam int CW  = 72;

    logic clk;
    logic rst_n;

    initial clk = 1'b0;
    always #5 clk = ~clk;    // 100 MHz

    initial begin
        rst_n = 1'b0;
        repeat (4) @(posedge clk);
        rst_n = 1'b1;
    end

    secded_ecc_if #(.DW(DW), .HAM(HAM), .CW(CW)) vif (.clk(clk), .rst_n(rst_n));

    secded_ecc #(.DW(DW), .PIPE(2)) dut (
        .clk          (clk),
        .rst_n        (rst_n),
        .in_valid     (vif.in_valid),
        .in_op        (vif.in_op),
        .in_data      (vif.in_data),
        .in_code      (vif.in_code),
        .out_valid    (vif.out_valid),
        .out_op       (vif.out_op),
        .out_code     (vif.out_code),
        .out_data     (vif.out_data),
        .out_syndrome (vif.out_syndrome),
        .out_sbe      (vif.out_sbe),
        .out_dbe      (vif.out_dbe)
    );

    initial begin
        uvm_config_db#(virtual secded_ecc_if)::set(null, "*", "vif", vif);
        $dumpfile("tb_top.vcd");
        $dumpvars(0, tb_top);
        run_test();
    end

    // Global watchdog independent of UVM objections.
    initial begin
        #1ms;
        `uvm_fatal("TIMEOUT", "global watchdog fired")
    end
endmodule
